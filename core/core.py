...
def streebog_256(data: bytes) -> bytes:
    ...
    """
    h = gost34112012.GOST34112012(data, digest_size=32)
    return h.digest()
...
def sign(data: bytes, private_key: int) -> Tuple[int, int]:
    """
...
def verify(data: bytes, public_key: Point, signature: Tuple[int, int]) -> bool:
    """
